...
# ===========================================================
# FINGERPRINT STORE — import and initialise
# fp_store.py must be in the same folder as this script
# ===========================================================
import fp_store
fp_store.init(
    zk_instance     = zk,
    zoho_request_fn = lambda method, url, **kw: zoho_request(method, url, **kw),
    auth_headers_fn = lambda: auth_headers(),
        ...
        if r.status_code == 200:
            ...
            if "access_token" in res:
                ...
                if err == "invalid_client":
                    _log.error(">>> invalid_client: CLIENT_ID or CLIENT_SECRET wrong.")
                    return None
                if err in ("invalid_code", "access_denied"):
                    _log.error(">>> Refresh token revoked or expired.")
                    return None
        else:
...
def auth_headers():
    ...
    if not token:
        _log.error("auth_headers: no token available")
        return {}
...
def find_worker(zk_user_id, force_refresh: bool = False):
    ...
    if r and r.status_code == 200:
        for w in r.json().get("data", []):
            ...
            if uid in (zk_val, wid_val, zk_val.split(".")[0], wid_val.split(".")[0]):
                _wcache_set(uid, w)
                return w
    _log.error(f"find_worker({uid}): NOT found after all attempts")
    return None
...
def search_workers_by_name(name_query: str) -> list:
    ...
    try:
        r = zoho_request("GET", url, headers=hdrs)
        if r and r.status_code == 200:
            return [w for w in r.json().get("data", [])
    ...
    if action == "checkin":
        ...
        if r_u and r_u.status_code == 200:
            ...
            if code == 3000:
                ...
                save_lock(lock)
                _log.info(f"CHECKOUT OK: {full_name} hours={round(total_hours, 2)}")
...
def run_auto_checkout(gui_log_fn=None, done_cb=None):
    ...
    if done_cb: done_cb(success_names, fail_names)
...
# (FingerprintCanvas, PulseLED, DonutRing, AdminPanel classes are unchanged — paste from original)
# ===========================================================
# FINGERPRINT CANVAS  (unchanged)
# ===========================================================
...
# ===========================================================
# ADMIN PANEL  (unchanged except adding fingerprint tab)
# ===========================================================
...
class FingerprintGUI:
    def __init__(self, root):
        ...
        # Load fingerprint templates from Zoho in the background
        def _bg_fp_sync():
            try:
                imported, skipped = fp_store.sync_from_zoho(
                    progress_cb=lambda m: self.log(m, "info"))
                self._gui(lambda: self.log(
                    f"Fingerprints loaded: {imported} templates from Zoho "
                    f"({skipped} had none)", "ok"))
            except Exception as e:
                self._gui(lambda: self.log(f"FP sync warning: {e}", "warn"))
        threading.Thread(target=_bg_fp_sync, daemon=True).start()
        ...
        def _check():
            ...
            if not token:
                self._gui(lambda: self.log(
                    ...
                    "check CLIENT_ID / CLIENT_SECRET / REFRESH_TOKEN in .env", "err"))
    ...
    def _build_left(self, parent):
        ...
        self._avatar_circle = self._avatar_cv.create_oval(2, 2, 46, 46,
                                                           fill=BORDER, outline="")
        self._avatar_text   = self._avatar_cv.create_oval(24, 24, text="",
    ...
    def _animate_finlanza(self):
        try:
            ...
            g = max(140, min(g, 255))
            b = max(180, min(b, 255))
            self._finlanza_canvas.itemconfig(
                self._finlanza_text, fill=f"#{r:02x}{g:02x}{b:02x}")
            self.root.after(30, self._animate_finlanza)
        except Exception:
            pass
    ...
    # ===========================================================
    # _process — fingerprint MUST match the typed ID
    # ===========================================================
    def _process(self, uid: str, action: str):
        success = False; msg = ""; full_name = uid
        try:
            ...
            # The scanned finger must match the stored template for this ID.
            # If the ID is not enrolled, or the wrong person scans, we BLOCK.
            self.log("Place your finger on the scanner…", "info")

            verified, fp_msg, fp_name = fp_store.verify(uid)

            if not verified:
                # Show the exact mismatch reason on screen
                self.log(fp_msg, "err")
                self._gui(lambda: self._scan_err("BLOCKED"))
                _m = fp_msg  # capture for lambda
                self._gui(lambda: self._show_flash(
                    "✗", "Access Denied", _m, "", RED_DIM))
                return

            # Identity confirmed
            full_name = fp_name
            self._gui(self._scan_ok)
            self.log(f"Fingerprint matched: {full_name} ✔", "ok")
            # ─────────────────────────────────────────────────────────────────
            ...
            if not worker:
                ...
                self._gui(lambda: self._show_flash(
                    ...
                    "Check attendance.log for diagnostics.", "", RED_DIM))
                return

            # Use the name from Zoho (more reliable than FP store)
            full_name = worker.get("Full_Name", full_name)
            self.log(f"Worker confirmed: {full_name}", "ok")
